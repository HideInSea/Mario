...
cc.Class({
    ...
    init(){
        ...
        if(config.mapConfig.startType[globalMgr.getRound()]){
    ...
    update (dt) {
        //检测玩家是否走到终点
        if(this.endPos&&gm.currPlayer.x>=this.endPos.x&&gm.currPlayer.x<=this.endPos.x+20&&!globalMgr.isGameOver){
            gm.currPlayer.active=false;
            globalMgr.isGameOver=true;
    ...
    vPipeEntry(type,pos){
        ...
        if(args[0]===ENUM.POSITION_TYPE.ROUND){
            ...
            globalMgr.setRound((parseInt(args[1])-1)*4);
            setTimeout(function(){
