...
cc.Class({
    ...
    init(){
        this.roadCheck=parseInt(arguments[2]);
        this.isFlip=!!arguments[3];

        let desObj=gm.posMgr.posLayer.getObject("conditionEnd_"+arguments[1]);
    ...
    interactive(actor){
        if(actor.node.x>this.node.x&&actor.node.x<this.node.x+20&&actor.node.y>=this.node.y&&actor.node.y<=this.node.y+3*40){
            ...
            if((this.roadCheck===actor.roadCheck&&!this.isFlip)||(this.isFlip&&this.roadCheck!==actor.roadCheck)){
